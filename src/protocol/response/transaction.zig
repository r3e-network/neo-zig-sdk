...
pub const Transaction = struct {
    ...
    pub fn encodeToJson(self: Self, allocator: std.mem.Allocator) ![]u8 {
        ...
        return try std.fmt.allocPrint(allocator, "{{\"hash\":\"{s}\",\"size\":{},\"version\":{},\"nonce\":{},\"sender\":\"{s}\",\"sysfee\":\"{s}\",\"netfee\":\"{s}\",\"validuntilblock\":{},\"script\":\"{s}\",\"blockhash\":{s},\"confirmations\":{s},\"blocktime\":{s}}}", .{
            hash_str,
            self.size,
            self.version,
            self.nonce,
    ...
    pub fn format(self: Self, allocator: std.mem.Allocator) ![]u8 {
        ...
        return try std.fmt.allocPrint(allocator, "Transaction(hash: {s}, size: {} bytes, signers: {}, status: {s})", .{ hash_str, self.size, self.signers.len, status });
    }
};
